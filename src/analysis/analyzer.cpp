...
std::any Analyzer::visitConstDef(CACTParser::ConstDefContext *ctx) {
    ...
    if (env->getCurrBlock()->getPre() == nullptr) {
        ...
        if (ctx->dimsize == 0) {
            ...
            irGenerator->addIntermediateCode(ctx->datatype, IR_L_ALLOC, dst, nullptr, nullptr);
        }
        else {
            int len = accumulate(ctx->dimarr.begin(), ctx->dimarr.end(), 1, std::multiplies<int>());
    ...
    ValSymbol *valSymbol = env->getCurrBlock()->addVariable(ctx->datatype, ctx->symbolname, ctx->dimsize, ctx->dimarr, true);
...
std::any Analyzer::visitVarDef(CACTParser::VarDefContext *ctx) {
    ...
    if (ctx->EQUALS() != nullptr) {
        ...
        if (ctx->datatype == INT)
            newinitval = irGenerator->addIRImmediateValue(ctx->datatype, std::to_string(0));
        else if (ctx->datatype == FLOAT)
            newinitval = irGenerator->addIRImmediateValue(ctx->datatype, std::to_string(0.0f));
        else if (ctx->datatype == DOUBLE)
            newinitval = irGenerator->addIRImmediateValue(ctx->datatype, std::to_string(0.0));
        else
            newinitval = irGenerator->addIRImmediateValue(ctx->datatype, std::string("false"));
    ...
    ValSymbol *valSymbol = env->getCurrBlock()->addVariable(ctx->datatype, ctx->symbolname, ctx->dimsize, ctx->dimarr, false);
    valSymbol->setIROperand(dst); 
    return std::any();
}
...
std::any Analyzer::visitConstInitValBasic(CACTParser::ConstInitValBasicContext *ctx) {
    ...
    if (ctx->constExp()->datatype != ctx->datatype) {
        ...
        IROperand *newInitVal = irGenerator->addIRImmediateValue(ctx->constExp()->datatype, ctx->constExp()->value);
        ctx->initval.push_back(newInitVal);
    }
    return std::any();
}
...
std::any Analyzer::visitConstExp(CACTParser::ConstExpContext *ctx) {
    if (ctx->BOOLCONST() != nullptr) {
        ...
        ctx->result   = irGenerator->addIRImmediateValue(ctx->datatype, ctx->value);
    }
    else {
...
std::any Analyzer::visitFuncDef(CACTParser::FuncDefContext *ctx) {
    ...
    //add to symboltable, add FUNC_START IntermediateCode
    ...
    irGenerator->addIntermediateCode(ctx->funcType()->functype, IR_FUNC_START, newOperand, nullptr, nullptr);
    ...
    //add FUNC_END IntermediateCode
    irGenerator->addIntermediateCode(ctx->funcType()->functype, IR_FUNC_END, newOperand, nullptr, nullptr);
    return std::any();
}
...
std::any Analyzer::visitPriExpNum(CACTParser::PriExpNumContext *ctx) {
    ...
    ctx->result   = irGenerator->addIRImmediateValue(ctx->datatype, ctx->num()->value);
    return std::any();
}
...
std::any Analyzer::visitLVal(CACTParser::LValContext *ctx) {
    ...
    if (valSymbol->getDimSize() == 0) {
        ...
        IROperand *newTempSum = irGenerator->addTemporaryValue(Type::INT);
        IROperand *newTempSum1;
        IROperand *newTempProduct;
        IROperand *newImmediate = irGenerator->addIRImmediateValue(Type::INT, std::to_string(0));
        irGenerator->addIntermediateCode(Type::INT, IR_ADD, newTempSum, newImmediate, newImmediate);
        for (int i = 0; i < ctx->LBRACKET().size(); i++) {
            ...
            newImmediate = irGenerator->addIRImmediateValue(Type::INT, std::to_string(nestedNum * byteNum));
            ...
            newTempProduct = irGenerator->addTemporaryValue(Type::INT);
            newTempSum1 = irGenerator->addTemporaryValue(Type::INT);
            irGenerator->addIntermediateCode(Type::INT, IR_MUL, newTempProduct, ctx->exp(i)->result, newImmediate);
            irGenerator->addIntermediateCode(Type::INT, IR_ADD, newTempSum1, newTempSum, newTempProduct);
            newTempSum = newTempSum1;
        }    
        if (ctx->dimsize == 0) {
            ctx->result = irGenerator->addArrayElement(valSymbol->getIROperand(), newTempSum);
        }
        else {
...
std::any Analyzer::visitUnaryExpuna(CACTParser::UnaryExpunaContext *ctx) {
    ...
    if (ctx->ADD()) {
        ...
        ctx->result   = irGenerator->addTemporaryValue(ctx->datatype);
        if (ctx->SUB()) {
            irGenerator->addIntermediateCode(ctx->datatype, IR_NEG, ctx->result, ctx->unaryExp()->result, nullptr);
        }
        else {
            irGenerator->addIntermediateCode(ctx->datatype, IR_NOT, ctx->result, ctx->unaryExp()->result, nullptr);
        }
    }
    return std::any();
}
...
std::any Analyzer::visitUnaryExpfun(CACTParser::UnaryExpfunContext *ctx) {
    ...
    IROperand *newTemporaryValue;
    if (funcSymbol->getDataType() != Type::VOID) {
        newTemporaryValue = irGenerator->addTemporaryValue(funcSymbol->getDataType());
    }
    else {
        newTemporaryValue = nullptr;
    }
    irGenerator->addIntermediateCode(funcSymbol->getDataType(), IR_CALL, newIRFunc, newTemporaryValue, nullptr);
    ctx->datatype = funcSymbol->getDataType();
    ctx->dimsize  = 0;
    ctx->result = newTemporaryValue;
    return std::any();
}
...
std::any Analyzer::visitFuncRParams(CACTParser::FuncRParamsContext *ctx) {
    ...
    for (int i = 0; i < params.size(); i++) {
        ...
        irGenerator->addIntermediateCode(exps[i]->datatype, IR_PARAM, exps[i]->result, nullptr, nullptr);
    }
    return std::any();
}
...
std::any Analyzer::visitMulExpmul(CACTParser::MulExpmulContext *ctx) {
    ...
    if (dynamic_cast<IRImmediateValue*>(ctx->mulExp()->result) == nullptr || dynamic_cast<IRImmediateValue*>(ctx->unaryExp()->result) == nullptr) {
        ctx->result = irGenerator->addTemporaryValue(ctx->datatype);
        if (ctx->MUL() != nullptr) {
            irGenerator->addIntermediateCode(ctx->datatype, IR_MUL, ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result);
        }
        else if (ctx->DIV() != nullptr) {
            irGenerator->addIntermediateCode(ctx->datatype, IR_DIV, ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result);
        }
        else {
            irGenerator->addIntermediateCode(ctx->datatype, IR_MOD, ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result);
        }
    }
    else {
        if (ctx->datatype == INT) {
            int op1 = stoi(reinterpret_cast<IRImmediateValue*>(ctx->mulExp()->result)->val);
            int op2 = stoi(reinterpret_cast<IRImmediateValue*>(ctx->unaryExp()->result)->val);
            if (ctx->MUL() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(INT, std::to_string(op1 * op2));
            else if (ctx->DIV() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(INT, std::to_string(op1 / op2));
            else
                ctx->result = irGenerator->addIRImmediateValue(INT, std::to_string(op1 % op2));
            ...
            float op1 = stof(reinterpret_cast<IRImmediateValue*>(ctx->mulExp()->result)->val);
            float op2 = stof(reinterpret_cast<IRImmediateValue*>(ctx->unaryExp()->result)->val);
            if (ctx->MUL() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(FLOAT, std::to_string(op1 * op2));
            else if (ctx->DIV() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(FLOAT, std::to_string(op1 / op2));
        }
        else{
            double op1 = stod(reinterpret_cast<IRImmediateValue*>(ctx->mulExp()->result)->val);
            double op2 = stod(reinterpret_cast<IRImmediateValue*>(ctx->unaryExp()->result)->val);
            if (ctx->MUL() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(DOUBLE, std::to_string(op1 * op2));
            else if (ctx->DIV() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(DOUBLE, std::to_string(op1 / op2));
        }
    }
...
std::any Analyzer::visitAddExpadd(CACTParser::AddExpaddContext *ctx) {
    ...
    if (dynamic_cast<IRImmediateValue*>(ctx->addExp()->result) == nullptr || dynamic_cast<IRImmediateValue*>(ctx->mulExp()->result) == nullptr) {
        ctx->result = irGenerator->addTemporaryValue(ctx->datatype);
        if (ctx->ADD() != nullptr) {
            irGenerator->addIntermediateCode(ctx->datatype, IR_ADD, ctx->result, ctx->addExp()->result, ctx->mulExp()->result);
        }
        else {
            irGenerator->addIntermediateCode(ctx->datatype, IR_SUB, ctx->result, ctx->addExp()->result, ctx->mulExp()->result);
        }
    }
    else {
        if (ctx->datatype == INT) {
            int op1 = stoi(reinterpret_cast<IRImmediateValue*>(ctx->addExp()->result)->val);
            int op2 = stoi(reinterpret_cast<IRImmediateValue*>(ctx->mulExp()->result)->val);
            if (ctx->ADD() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(INT, std::to_string(op1 + op2));
            else
                ctx->result = irGenerator->addIRImmediateValue(INT, std::to_string(op1 - op2));
            ...
            float op1 = stof(reinterpret_cast<IRImmediateValue*>(ctx->addExp()->result)->val);
            float op2 = stof(reinterpret_cast<IRImmediateValue*>(ctx->mulExp()->result)->val);
            if (ctx->ADD() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(FLOAT, std::to_string(op1 + op2));
            else
                ctx->result = irGenerator->addIRImmediateValue(FLOAT, std::to_string(op1 - op2));
            ...
            double op1 = stod(reinterpret_cast<IRImmediateValue*>(ctx->addExp()->result)->val);
            double op2 = stod(reinterpret_cast<IRImmediateValue*>(ctx->mulExp()->result)->val);
            if (ctx->ADD() != nullptr)
                ctx->result = irGenerator->addIRImmediateValue(DOUBLE, std::to_string(op1 + op2));
            else
                ctx->result = irGenerator->addIRImmediateValue(DOUBLE, std::to_string(op1 - op2));
...
std::any Analyzer::visitStmtAssign(CACTParser::StmtAssignContext *ctx) {
    ...
    irGenerator->addIntermediateCode(ctx->lVal()->datatype, IR_ASSIGN, ctx->lVal()->result, ctx->exp()->result, nullptr);
    if (ctx->next == nullptr) {
...
std::any Analyzer::visitStmtReturn(CACTParser::StmtReturnContext *ctx) {
    if (ctx->exp() != nullptr) {
        ...
        irGenerator->addIntermediateCode(ctx->exp()->datatype, IR_RETURN, ctx->exp()->result, nullptr, nullptr);
    }
    else {
        if (env->getCurrFunc()->getDataType() != Type::VOID) {
            throw std::runtime_error("the data type of return value mismatched with funtion definition.");
        }
        irGenerator->addIntermediateCode(Type::VOID, IR_RETURN, nullptr, nullptr, nullptr);
    }
    if (ctx->next == nullptr) {
        reinterpret_cast<CACTParser::BlockItemStmtContext*>(ctx->parent)->newLabel = nullptr;
    }
...
std::any Analyzer::visitStmtIf(CACTParser::StmtIfContext *ctx) {
    ...
    if (ctx->next == nullptr) {
        ctx->next = irGenerator->createLabel();
    ...
    ctx->stmt()->accept(this);
    if (addNewLabel) {
        irGenerator->addIntermediateCode(Type::VOID, IR_LABEL, ctx->next, nullptr, nullptr);
    }
    return std::any();
}
...
std::any Analyzer::visitStmtIfElse(CACTParser::StmtIfElseContext *ctx) {
    ...
    ctx->lOrExp()->True  = nullptr; //fall
    ctx->lOrExp()->False = irGenerator->createLabel();
    ctx->stmt(1)->before = ctx->lOrExp()->False;
    ctx->lOrExp()->accept(this);
    ctx->stmt(0)->accept(this);
    irGenerator->addIntermediateCode(Type::VOID, IR_GOTO, ctx->next, nullptr, nullptr);
    irGenerator->addIntermediateCode(Type::VOID, IR_LABEL, ctx->lOrExp()->False, nullptr, nullptr);
...
std::any Analyzer::visitStmtWhile(CACTParser::StmtWhileContext *ctx) {
    ...
    if (ctx->before == nullptr) {
        ctx->before = irGenerator->createLabel();
        irGenerator->addIntermediateCode(Type::VOID, IR_LABEL, ctx->before, nullptr, nullptr);
    }
    ...
    ctx->lOrExp()->accept(this);
    ctx->stmt()->accept(this);
    irGenerator->addIntermediateCode(Type::VOID, IR_GOTO, ctx->before, nullptr, nullptr);
    if (addNewLabel) {
        irGenerator->addIntermediateCode(Type::VOID, IR_LABEL, ctx->next, nullptr, nullptr);
    }
...
std::any Analyzer::visitStmtBreak(CACTParser::StmtBreakContext *ctx) {
    ...
    irGenerator->addIntermediateCode(Type::VOID, IR_GOTO, currWhile->breakDst, nullptr, nullptr);
    return std::any();
}
...
std::any Analyzer::visitStmtContinue(CACTParser::StmtContinueContext *ctx) {
    ...
    irGenerator->addIntermediateCode(Type::VOID, IR_GOTO, currWhile->contDst, nullptr, nullptr);
    return std::any();
}
...
std::any Analyzer::visitLOrExpand(CACTParser::LOrExpandContext *ctx) {
    if (ctx->first) {
        ...
        ctx->lAndExp()->True  = irGenerator->createLabel();
...
std::any Analyzer::visitLOrExpor(CACTParser::LOrExporContext *ctx) {
    ...
    ctx->lAndExp()->accept(this);
    if (ctx->first) {
        irGenerator->addIntermediateCode(Type::VOID, IR_LABEL, ctx->newLabel, nullptr, nullptr);
    }
    return std::any();
}
...
std::any Analyzer::visitLAndExpeq(CACTParser::LAndExpeqContext *ctx) {
    if (ctx->first) {
        ...
        if (ctx->True == nullptr) {
            ...
            ctx->eqExp()->True  = nullptr;
            ctx->eqExp()->False = irGenerator->createLabel();
...
std::any Analyzer::visitEqExpeq(CACTParser::EqExpeqContext *ctx) {
    ...
    if (ctx->EQ()) {
        if (ctx->True == nullptr) {
            irGenerator->addIntermediateCode(ctx->relExp(0)->datatype, IR_BNE, ctx->False, ctx->relExp(0)->result, ctx->relExp(1)->result);
        }
        else {
            irGenerator->addIntermediateCode(ctx->relExp(0)->datatype, IR_BEQ, ctx->True, ctx->relExp(0)->result, ctx->relExp(1)->result);
        }
    }
    else {
        if (ctx->True == nullptr) {
            irGenerator->addIntermediateCode(ctx->relExp(0)->datatype, IR_BEQ, ctx->False, ctx->relExp(0)->result, ctx->relExp(1)->result);
        }
        else {
            irGenerator->addIntermediateCode(ctx->relExp(0)->datatype, IR_BNE, ctx->True, ctx->relExp(0)->result, ctx->relExp(1)->result);
        }
    }
    return std::any();
}
...
std::any Analyzer::visitRelExprel(CACTParser::RelExprelContext *ctx) {
    ...
    if (ctx->jump) {
        if (ctx->LT()) {
            if (ctx->True == nullptr) {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BGE, ctx->False, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
            else {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BLT, ctx->True, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
        }
        else if (ctx->GT()) {
            if (ctx->True == nullptr) {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BLE, ctx->False, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
            else {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BGT, ctx->True, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
        }
        else if (ctx->LE()) {
            if (ctx->True == nullptr) {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BGT, ctx->False, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
            else {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BLE, ctx->True, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
        }
        else {
            if (ctx->True == nullptr) {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BLT, ctx->False, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
            else {
                irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_BGE, ctx->True, ctx->addExp(0)->result, ctx->addExp(1)->result);
            }
        }
    }
    else {
        ctx->result = irGenerator->addTemporaryValue(Type::BOOL);
        if (ctx->LT()) {
            irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_SLT, ctx->result, ctx->addExp(0)->result, ctx->addExp(1)->result);
        }
        else if (ctx->GT()) {
            irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_SLT, ctx->result, ctx->addExp(1)->result, ctx->addExp(0)->result);
        }
        else if (ctx->LE()) {
            irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_SLTE, ctx->result, ctx->addExp(0)->result, ctx->addExp(1)->result);
        }
        else if (ctx->GE()) {
            irGenerator->addIntermediateCode(ctx->addExp(0)->datatype, IR_SLTE, ctx->result, ctx->addExp(1)->result, ctx->addExp(0)->result);
        }
    }
    return std::any();
}
...
// 在块中添加变量符号
ValSymbol *Block::addVariable(Type dataType, std::string &symbolName, int dimSize, std::vector<int> &dimArray, bool isConst) {
