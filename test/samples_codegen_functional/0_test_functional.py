...
"""
lib_path ="/home/xinyang/labs/cact-master-1/cact-master/test/samples_codegen_functional"
...
# if use  to compile, add these declarations
...
def test(use_gcc = False):
    ...
    for cact_file in sorted(cact_files):
        ...
        if use_gcc:
            ...
            ret = subprocess.run([compiler_path, "-O0", cact_file, "-S", "-o", s_file])
        if ret.returncode != 0:
        ...
        if os.path.exists(in_file):
            ret = subprocess.run(["spike", pk_path, "./"+exe_file], stdin=open(in_file, 'r'), stdout=subprocess.PIPE)
        else:
            ret = subprocess.run(["spike", pk_path, "./"+exe_file], stdout=subprocess.PIPE)
